...
public class DummyGame implements IGameLogic {
    ...
    public void init(Window window) throws Exception {
        ...
        house.setScale(nextFloatRange(0.010f, 0.500f));
        t = house;
...
// my hacked together code to grab last added object
//        GameItem lastItem = gameItems.get(gameItems.size()-1); //list works best mar 8th
//        GameItem lastItem = gameItems[gameItems.length-1]; //arraylist
//        GameItem lastItem = gameItems.get(gameItems.size()-1); //list


//        for (GameItem objs : gameItems){
//            if (objs.mostRecent == true){
//                objs.mostRecent = false;
////                lastItem.mostRecent = true;
//            }
//            lastItem.mostRecent = true;
//
////            objs.mostRecent = false;
        ...
        for (GameItem outer : gameItems) {
            for (GameItem inner : gameItems) {
                if (outer != inner) {
                    ...
                    if (collide) {
                        outer.rebound();
                        System.out.println("COLLISION:\t\t inner x: " + inner.position.x + "\t\ty: " + inner.position.y + "\t\tz: " + inner.position.z);
...
//        Plane PlaneshapeTest01 = null;
//        Plane PlaneshapeTest02 = null;
        try {
...
//            Mesh[] addMesh = StaticMeshesLoader.load("src/main/resources/models/russ/russ9.obj", "src/main/resources/models/russ");
            ...
            PlaneshapeTest01.setPosition(-15.00f, 10.000f, 0.000f);
...
//    }
    ...
    List<GameItem> testItems = new ArrayList<>();
        public void timedLog(){
            for (GameItem item : testItems){
//                System.out.println("item.getPosition()" + item.getPosition());
                final float x = item.position.x;
                final float y = item.position.y;
                final float z = item.position.z;
                System.out.println("item: " + item + "\tx: " + x + "\ty: " + y + "\tz: " + z);
